...
class Editor {
  ...
  attach(observer) {
    ...
    console.log("editor получил observer");
  }
  detach(observer) {
    const observerIndex = this.#observers.indexOf(observer);

    if (observerIndex === -1) {
      return console.log("observer не найден");
    }
    // находим по индексу и удаляем 1 элемент
    this.#observers.splice(observerIndex, 1);
    console.log("editor удалил observer");
  }
...
editor1.createPost("my title", "my subtitle");

// Концепция ООП

// 1) абстракция - любой class, создает нечто
// 2) инкапсуляция - это нечто инкапсулируется, в единую сущность объединяются методы
// 3) наследование - прототипное наследование
// 4) полиморфизм - функции или классы, могут работать с разными видами данных
